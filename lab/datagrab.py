...
for keyword in keywords:
    ...
    press('f12')
    wait(1)
    js = 'let target = document.getElementsByClassName("see-all");target[0].children[1].click();let rows = document.getElementsByTagName("table")[2].tBodies[0].children;let arr = [...rows];for(let i =0;i<arr.length;i++){arr[i].children[0].getElementsByTagName("a")[1].click();};document.getElementById("button_Update").click();let url = window.location.href;let list = url.substr(67).split(",");function down(list) {let tmp = document.createElement("a");tmp.href = "data:text/plain;charset=utf-8," + encodeURI(list);tmp.download = "location_list";tmp.click();};down(list);'
    copy_val(js)
    paste_val()
    wait(0.5)
    press('enter')
    ...
    text_file = open("C:/Users/Arthur/Downloads/location_list.txt", "r")
